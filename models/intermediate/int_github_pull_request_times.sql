...
pull_request_review as (
    ...
    select * from {{ ref('stg_github_pull_request_review') }}
...
requested_reviewer_history as (
    ...
    select * from {{ ref('stg_github_requested_reviewer_history') }}
    where not removed
...
first_request_time as (
    ...
    select
        ...
        end) as time_of_first_requested_reviewer_review,
...
select
    ...
    {{ dbt_utils.datediff(
        ...
        'second') }} / 60 / 60 as hours_request_review_to_first_review,
    {{ dbt_utils.datediff(
          'time_of_first_request',
          "least(coalesce(time_of_first_requested_reviewer_review, " ~ dbt_utils.current_timestamp() ~ "), coalesce(issue.closed_at, " ~ dbt_utils.current_timestamp() ~ "))",
          'second') }} / 60 / 60 as hours_request_review_to_first_action,
