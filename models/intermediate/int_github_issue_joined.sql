...
issue_labels as (
    ...
    select * from {{ ref('int_github_issue_labels')}}

),

repository_teams as (

    select * from {{ ref('int_github_repository_teams')}}

),

issue_assignees as (

    select * from {{ ref('int_github_issue_assignees')}}

),

issue_open_length as (

    select * from {{ ref('int_github_issue_open_length')}}

),

issue_comments as (

    select * from {{ ref('int_github_issue_comments')}}

),

creator as (

    select * from {{ ref('stg_github_user')}}

),

pull_request_times as (

    select * from {{ ref('int_github_pull_request_times')}}

),

pull_request_reviewers as (

    select * from {{ ref('int_github_pull_request_reviewers')}}
